...
template <typename T> class vector {
    public:
        ...
        // внутриклассовые типы переопределений (типичные для включения библиотеки STL), принятые для удобства восприятия класса кода:
        typedef T value_type;  // T определяет тип элемента (T — параметр шаблона)
        typedef T& reference;  // T& определяет тип ссылки на элемент
        typedef const T& const_reference;  // const T& определяет тип постоянной ссылки
        typedef T* iterator;  // T* определяет тип для перебора контейнера
        typedef const T* const_iterator;  // const T* определяет тип константы для перебора контейнера
        typedef std::size_t size_type;  // size_t определяет тип размера контейнера (стандартный тип — size_t)
        ...
        explicit vector(size_type n);  // параметризованный конструктор, создает вектор размера n
        vector(std::initializer_list<value_type> const &items);  // конструктор списка инициализаторов, создает вектор, инициализированный с использованием std::initializer_list
        vector(const vector &v);  // copy конструктор 
        vector(vector &&v) noexcept;  // move конструктор
        ~vector();  // деструктор
        vector<T> &operator=(vector &&v);  // Перегрузка оператора присваивания для move объекта
        ...
        void reserve(size_type size);  // выделяет хранилище элементов размера и копирует текущие элементы массива в вновь выделенный массив
        size_type capacity();  // возвращает количество элементов, которые могут храниться в выделенном на данный момент хранилище
        void shrink_to_fit();  // уменьшает использование памяти за счет освобождения неиспользуемой памяти

        // методы для изменения контейнера:
        void clear() noexcept;  // очищает содержимое
        iterator insert(iterator pos, const_reference value);  // вставляет элементы в конкретную позицию и возвращает итератор, указывающий на новый элемент
        ...
        // + 2 допа
        template <typename... Args> void insert_many_back(Args&&... args);  // добавляет новые элементы в конец контейнера
        template <typename... Args> iterator insert_many(const_iterator pos, Args&&... args);  // вставляет новые элементы в контейнер непосредственно перед pos
...
template <typename T> vector<T>::vector() : capacity_{0}, size_{0}, data_{nullptr}  { };

template <typename T> vector<T>::vector(size_type n) : /*vector(),*/ capacity_{n}, size_{n}, data_{nullptr} {
    // exeption выкидывает oper new
    // проверка n на ноль data_ = nullptr
    if(n != 0){
        data_ = reinterpret_cast<value_type *>(operator new(n * sizeof(value_type)));
...
template <typename T> vector<T>::vector(std::initializer_list<value_type> const &items) : vector(items.size()) {
...
template <typename T> vector<T>::vector(const vector &v) : vector(v.size_){
    if(size_ != 0){
        for(size_t i = 0; i < size_; i++){
            data_[i] = v.data_[i];
        }
...
template<typename T> typename vector<T>::reference vector<T>::at(size_type pos) {
    if (pos >= size()) throw std::out_of_range("Index out range");
    return data_[pos];
}

template<typename T> typename vector<T>::reference vector<T>::operator[](size_type pos) {
    // return data_[pos];
...
template<typename T> typename vector<T>::value_type* vector<T>::data() {
    return begin();
}
...
template<typename T> void vector<T>::reserve(size_type size) {
    ...
    tmp.data_ = reinterpret_cast<value_type *>(operator new(size * sizeof(value_type)));
...
template<typename T> typename vector<T>::iterator vector<T>::insert(iterator pos, const_reference value) {
    size_type rev_indx = end() - pos - 1;
    size_type insert_indx = pos - begin();
    if (insert_indx > size_) {
        throw std::out_of_range("InsertError: The insertion position is out of range of the vector ");
    }
    push_back(value);
    std::cout << "insert " << rev_indx << " "<< insert_indx << " " << size() << std::endl;
...
//  проверка на size = 0
    ...
    std::cout << "eras " << pos - begin() << "size " << size_ - position << std::endl;
    // (data_ + position)->~T();
...
template<typename T> void vector<T>::pop_back() {
    if(empty()) throw std::length_error("vector::pop_back called for empty vector");
    (data_ + size() - 1)->~T();    // ????
    size_--;
}
...
template<typename T> void vector<T>::get_new_capacity() {
    size_t new_capacity = capacity_;
    std::cout << "\n in push 0  " << new_capacity << "  " << capacity_ << "  " << size_ << std::endl;
...
template<typename T>
template<typename... Args> void vector<T>::insert_many_back(Args&&... args) {
    for (T val : std::initializer_list<T>{args...}) push_back(val);
}

template<typename T>
template <typename... Args> typename vector<T>::iterator vector<T>::insert_many(const_iterator pos, Args&&... args) {
    ...
    if (size() + args_qnt >= capacity()) {
        ...
        reserve(new_capacity);
        std::cout << "insert_many_2 " << args_qnt << "  " << capacity() << "  " << begin() << "  " << end() << "  " << pos << std::endl;
        std::cout << begin() + insert_indx << std::endl;
    }
    ...
    // size_ += args_qnt;
    return begin() + insert_indx;
}

template<typename T> void vector<T>::move_range_to_end(size_t pos) {
    size_t range_size =  size() - pos;
    std::cout << "to_end_1 " << range_size << "  " << capacity() << "  "<< size()  << std::endl;
